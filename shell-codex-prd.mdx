...
| v1 behavior | Disposition | Reason | v2 owner | Validation |
|---|---|---|---|---|
| permissions role primitives | Replicate now | shell-core | packages/permissions | unit tests |
| app-local route ACL hardcoding | Retire | coupling risk | packages/permissions + middleware | static checks |
| manifest normalize/validate/assess | Replicate now | module infra backbone | packages/module-sdk | contract tests |
| hardcoded fallback app manifests | Retire | must be registry-driven | module_registry seed policy | empty-registry tests |
| import assessment reports | Replicate now | admin operations value | module-sdk + platform admin | integration tests |
| deep internal package imports | Retire | boundary fragility | all packages | lint rule |
| db browser/server/admin wrappers | Replicate now | proven reusable shell pattern | packages/db | integration tests |
| integration inheritance resolver pattern | Replicate now (genericized) | strong tenancy behavior | packages/db + theme | inheritance tests |
| fragmented theming catalogs | Retire | single source of truth required | packages/theme | token consistency tests |
| tenancy context + impersonation rules | Replicate now | operator-critical | permissions + db | scope/impersonation tests |
| chat runtime behavior | Defer to modules phase | module business logic | n/a | shell code review |
| kb runtime/worker behavior | Defer to modules phase | module business logic | n/a | shell code review |
| plasmic host/render | Retire | out of shell principles | n/a | dependency scan |
| duplicate route variants/.old files | Retire | maintainability risk | repo hygiene policy | CI file checks |
